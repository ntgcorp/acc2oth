...
**DA estrarre (solo se richiesto da flag `EXP.*`):** Form, Moduli standard (`.bas`), Moduli di classe (`.cls`), Query (`.sql`), Macro (`.txt`), Report (`.txt`), Tabelle (`.csv`).
**NON estrarre di default:** Tabelle (solo log `Skipped` se `EXP.TABLES=False`).
...
; Esporta Tabelle in \tables come .csv (iterazione TableDefs; ignora MSys*)
EXP.TABLES=True o False
EXP.TABLES.LIST=Tabella1, Tabella2   ; opzionale: filtra solo questi nomi (CSV, case-insensitive)

; Genera file Java per ogni Classe/Modulo esportato (via OpenRouter)
EXP.JAVA=True o False
EXP.JAVA.LIST=Modulo1, Classe1   ; opzionale: filtra solo questi nomi (CSV, case-insensitive)
...
- `EXP.TABLES.LIST` richiede `EXP.TABLES=True` (se `LIST` presente ma flag `False` -> errore exit 2). Stesso vincolo per `EXP.JAVA.LIST` / `EXP.PROMPT.LIST` / `EXP.PYTHON.LIST`.
...
- Se presenti `EXP.PROMPT.LIST` / `EXP.PYTHON.LIST` / `EXP.JAVA.LIST` / `EXP.TABLES.LIST`: split su `,`, strip, upper, rimuovi vuoti, deduplica mantenendo ordine. Altrimenti lista vuota = "tutti".
- Valida dipendenze LIST: se `EXP.TABLES.LIST` non vuota ma `EXP.TABLES=False` -> errore exit 2. Stesso per `EXP.JAVA.LIST`/`EXP.PROMPT.LIST`/`EXP.PYTHON.LIST`.
...
#### `app_java(config: dict, logger) -> str`

- Analogo a `app_python` ma con `EXP.JAVA` / `EXP.JAVA.LIST`; scansiona `OUT.PATH/modules/*.bas` e `OUT.PATH/classes/*.cls` sul filesystem. Genera `.java` corrispondente per ogni `.bas`/`.cls` filtrato (stessa cartella, stesso basename con estensione `.java`). `EXP.JAVA.LIST` filtra solo questa fase e opera su filesystem.
- **Diagnostica identica a `app_prompt`** con prefisso `OpenRouter JAVA start/done` (operazione, model effettivo, src, chars prima; durata `Mm SSs`, token prompt/completion/total, costo `$` dopo).
- System prompt per Java: "Sei un transpiler VBA->Java. Traduci il modulo .bas/.cls in Java 17+ idiomatico, preservando la logica: classi, metodi, cicli, condizioni, gestione errori. Usa record/optional/stream dove appropriato. Sostituisci DAO/ADODB con JDBC/Hibernate e MsgBox/InputBox con logging/exceptions. Restituisci SOLO codice Java, senza spiegazioni."

#### `app_tables(config: dict, logger) -> str`

- Se `EXP.TABLES=False` -> skip (sResult ok).
- Altrimenti itera `CurrentDb.TableDefs`, ignora di sistema (`name.startswith("MSys")`). Per ogni tabella (o filtrata da `EXP.TABLES.LIST` CSV upper case-insensitive sul nome):
  - Crea subcartella `tables/` se assente.
  - Esporta in CSV (`utf-8`) con intestazione: `SELECT * FROM [Tabella]` via `CurrentDb.OpenRecordset`, scrive campi + righe.
  - Nome file: `tables/<sanitized>.csv`. Sanitizzazione come per gli altri oggetti.
  - Log per categoria e per oggetto (`INFO: Exporting TABLES (N)`, `INFO: Exported Tabella -> tables/Tabella.csv`).
  - Su errore singolo oggetto logga `ERROR` e prosegue (raccoglie in `index.json:errors`).
- **Ordine esecuzione**: `app_tables` viene chiamato **dopo** `app_export` e **prima** di `app_prompt`/`app_python`/`app_java`.
...
```json
{
  ...
  "counts": { "forms": 12, "modules": 5, "classes": 4, "queryes": 20, "macros": 2, "reports": 3, "tables": 15, "skipped_tables": 0 },
  "files": {
    "forms":   [{ "name": "Form_Clienti", "file_object": "forms/Form_Clienti.txt", "file_prompt": null }],
    "modules": [{ "name": "modUtility",   "file_object": "modules/modUtility.bas", "file_prompt": "modules/prompt_modUtility.md", "file_python": "modules/modUtility.py", "file_java": "modules/modUtility.java" }],
    "classes": [{ "name": "clsGestore",   "file_object": "classes/clsGestore.cls", "file_prompt": "classes/prompt_clsGestore.md", "file_python": null, "file_java": null }],
    ...
    "tables":  [{ "name": "Tabella1",     "file_object": "tables/Tabella1.csv",    "file_prompt": null }]
  },
  "errors": [{ "object_type": "Form", "object_name": "Form_Bad", "error": "SaveAsText failed: ..." }]
}
```

- `file_prompt` / `file_python` / `file_java` valorizzati solo se generati; altrimenti `null`. Path relativi a `OUT.PATH` con `/`.
- **Merge con `index.json` precedente**: se esiste, fonde le entry per `(categoria, name upper)` preservando lo storico (utile quando il run non esporta ma genera solo prompt/python/java da filesystem); le entry correnti aggiornano `file_object`/`file_prompt`/`file_python`/`file_java` se valorizzati. `counts` ricalcolati sul merge; `skipped_tables` del run se >0 altrimenti storico.
...
- Livelli: `INFO` (oggetti estratti/skipped, categorie), `WARNING` (nomi sanitizzati, alias QUERYES/QUERIES, overwrite implicito), `ERROR` (fallimento singolo oggetto — **continua** con gli altri, raccoglie in `index.json.errors`).
- Non interrompere l intera estrazione per un singolo oggetto fallito.
- Gestisci esplicitamente: file `.accdb` inesistente/non apribile, DB locked (`Could not use ... already in use`), Access/COM non installato, `OUT.PATH` non scrivibile, `LOG` non scrivibile, INI malformato / sezione `[CONFIG]` mancante, `OPENROUTER.*` mancanti nel `dictConfig` finale quando richiesti (`EXP.PROMPT`/`EXP.PYTHON`/`EXP.JAVA=True`, tenendo conto dell override CLI), HTTP 4xx/5xx OpenRouter (retry 1 volta poi logga errore e prosegue), sanitizzazione/collisioni nomi, `EXP.TABLES.LIST` senza `EXP.TABLES=True`, `EXP.JAVA.LIST` senza `EXP.JAVA=True`.

---

## 8. CRITERI DI ACCETTAZIONE (per validare il codice generato)

- [ ] `python -m acc2oth example.ini` crea `OUT.PATH/{forms,modules,classes,queryes,macros,reports,tables}/` + `LOG` + `index.json` con schema cap. 3.2
- [ ] Ogni Form/Report/Modulo/Classe/Query/Macro/Tabella del DB compare come file singolo con estensione corretta (conteggio vs `AllForms`/`AllModules`/`QueryDefs`/`TableDefs` filtrate)
- [ ] `LOG` contiene riga per categoria + per ogni oggetto esportato
- [ ] `index.json` valido con `name`/`file_object`/`file_prompt`/`file_python`/`file_java` per categoria e `errors` per fallimenti singoli
- [ ] `EXP.PROMPT.LIST` / `EXP.PYTHON.LIST` / `EXP.JAVA.LIST` filtrano solo generazione prompt/python/java, non l export grezzo
- [ ] `EXP.TABLES.LIST` filtra solo export tabelle CSV
- [ ] Sanitizzazione nomi e filtro QueryDef/TableDef di sistema (`~*`, `MSys*`)
- [ ] Chiusura pulita di `Access.Application.Quit()` anche su eccezione (verifica Task Manager)
- [ ] Testato su `.accdb` di esempio con almeno 2 Form, 1 Modulo standard, 1 Classe, 2 Query, 1 Macro, 1 Report, 1 Tabella
